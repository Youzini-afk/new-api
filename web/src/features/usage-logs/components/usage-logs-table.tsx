...
export function UsageLogsTable({ logCategory }: UsageLogsTableProps) {
  ...
  useEffect(() => {
    ...
    if (liveFeedContextRef.current !== liveFeedContextKey) {
      // A filter change can temporarily expose TanStack Query's previous-page
      // placeholder. Wait for the matching response instead of merging stale
      // rows from the old filter into the new live feed.
    ...
    if (!liveInsertTimerRef.current) drainLiveFeedQueue()
  }, [
    clearLiveFeedTimers,
    drainLiveFeedQueue,
    isAutoRefreshActive,
  ...
  const getRowId = useCallback(
    (row: Record<string, unknown>) =>
      logCategory === 'common'
        ? getUsageLogLiveKey(row as unknown as UsageLog)
        : String(row.id),
    [logCategory]
  )
