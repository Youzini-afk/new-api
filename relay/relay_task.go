...
func tryRealtimeFetch(c *gin.Context, task *model.Task, isOpenAIVideoAPI bool) []byte {
	...
	otherSettings := channelModel.GetOtherSettings()
	resp, err := service.DoChannelTrafficHTTPRequest(c.Request.Context(), channelModel.Id, otherSettings.TrafficControl, func() (*http.Response, error) {
		return adaptor.FetchTask(baseURL, channelModel.Key, map[string]any{
