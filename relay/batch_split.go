...
func executeRelayBatchChunks(c *gin.Context, info *relaycommon.RelayInfo, chunks []relayBatchChunk, concurrency int) ([]relayBatchHTTPResult, int, *types.NewAPIError) {
	...
	if batchContext.Err() != nil {
		return results, 0, types.NewOpenAIError(batchContext.Err(), types.ErrorCodeDoRequestFailed, http.StatusInternalServerError)
	}
...
func openAIErrorFromBatchBody(body []byte, statusCode int) *types.NewAPIError {
	...
	if apiErr := response.GetOpenAIError(); apiErr != nil && apiErr.Type != "" {
		return types.WithOpenAIError(*apiErr, statusCode)
	}
	return nil
}
...
func addRelayBatchUsage(target *dto.Usage, source dto.Usage) {
	...
	target.PromptTokensDetails.CacheWriteTokens += source.PromptTokensDetails.CacheWriteTokens
	...
	if source.InputTokensDetails != nil {
		...
		target.InputTokensDetails.CacheWriteTokens += source.InputTokensDetails.CacheWriteTokens
...
func batchPayloadError(err error) *types.NewAPIError {
	return types.NewErrorWithStatusCode(fmt.Errorf("failed to split batch request: %w", err), types.ErrorCodeInvalidRequest, http.StatusBadRequest, types.ErrOptionWithSkipRetry())
}

func batchMaxItemsError(kind string, itemCount, maxItems int) *types.NewAPIError {
	return types.NewErrorWithStatusCode(
		fmt.Errorf("%s request contains %d items, exceeding the configured limit of %d", kind, itemCount, maxItems),
...
func batchResponseError(chunkNumber int, err error) *types.NewAPIError {
	return types.NewOpenAIError(
		fmt.Errorf("batch chunk %d returned an invalid response: %w", chunkNumber, err),
