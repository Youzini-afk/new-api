...
func TestParseAndValidateRelayBatchSplitConfigRejectsInvalidValues(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			raw:  `{"version":1,"channel_ids":[3,3],"embedding":{"batch_size":25,"concurrency":1,"max_items":1000},"rerank":{"batch_size":25,"concurrency":1,"max_items":200}}`,
		},
		{
			name: "embedding batch too large",
			raw:  `{"version":1,"channel_ids":[],"embedding":{"batch_size":1001,"concurrency":1,"max_items":1000},"rerank":{"batch_size":25,"concurrency":1,"max_items":200}}`,
		},
		{
			name: "concurrency too large",
			raw:  `{"version":1,"channel_ids":[],"embedding":{"batch_size":25,"concurrency":5,"max_items":1000},"rerank":{"batch_size":25,"concurrency":1,"max_items":200}}`,
		},
		{
			name: "max below batch",
			raw:  `{"version":1,"channel_ids":[],"embedding":{"batch_size":25,"concurrency":1,"max_items":20},"rerank":{"batch_size":25,"concurrency":1,"max_items":200}}`,
		},
	}
...
func TestMatchRelayBatchSplitOnlyMatchesExplicitChannels(t *testing.T) {
	...
	assert.False(t, matched, "unselected channel must never be inferred from URL or name")

	_, matched = MatchRelayBatchSplit(17, RelayBatchKindRerank)
	assert.False(t, matched, "disabled request family must not match")
}
