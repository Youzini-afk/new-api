...
func discordBanPatrolCandidateQuery(ctx context.Context, now int64) *gorm.DB {
	return DB.WithContext(ctx).Model(&User{}).Where(
		"status = ? AND role < ? AND discord_gate_exempt = ? AND discord_id <> ? AND discord_refresh_token <> ? AND (discord_ban_patrol_retry_at = 0 OR discord_ban_patrol_retry_at <= ?)",
...
func FindDiscordBanPatrolCandidateUsers(ctx context.Context, limit int) ([]*User, error) {
	...
	err := discordBanPatrolCandidateQuery(ctx, common.GetTimestamp()).
		Order("CASE WHEN discord_ban_patrol_retry_at > 0 THEN 0 ELSE 1 END asc, discord_ban_patrol_retry_at asc, discord_ban_patrol_last_check_at asc, id asc").
		Limit(limit).
		Find(&users).Error
	return users, err
}
